...
std::unordered_set<const MapResource*> Map::GetAllResources() {
  ...
  for (auto& id : point_ids_)
    res.insert(
        dynamic_cast<const MapResource*>(object_pool_.find(id)->second.get()));
...
std::unordered_set<const Point*> Map::GetAllPoints() {
  ...
  for (auto& id : point_ids_)
    res.insert(dynamic_cast<const Point*>(object_pool_.find(id)->second.get()));
  return res;
}
...
std::unordered_set<const Path*> Map::GetAllPaths() {
  ...
  for (auto& id : path_ids_)
    res.insert(dynamic_cast<const Path*>(object_pool_.find(id)->second.get()));
  return res;
}
...
std::unordered_set<const Location*> Map::GetAllLocations() {
  ...
  for (auto& id : location_ids_)
    res.insert(
        dynamic_cast<const Location*>(object_pool_.find(id)->second.get()));
  return res;
}
...
std::unordered_set<const Block*> Map::GetAllBlocks() {
  ...
  for (auto& id : block_ids_)
    res.insert(dynamic_cast<const Block*>(object_pool_.find(id)->second.get()));
  return res;
}
...
std::unordered_set<const Vehicle*> Map::GetAllVehicles() {
  ...
  for (auto& id : vehicle_ids_)
    res.insert(
        dynamic_cast<const Vehicle*>(object_pool_.find(id)->second.get()));
  return res;
}
